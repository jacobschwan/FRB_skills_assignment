...
```{r}
high_data_fct %>%
    select(cycleendingbalance, ends_with("fct")) %>%
    pivot_longer(-cycleendingbalance, names_to = "group", values_to = "variables") %>%
    group_by(group, variables) %>%
    summarise(balance = sum(cycleendingbalance), .groups = "drop_last") %>%
    mutate(pct_balance = balance/sum(balance)) %>%
    arrange(group, variables)
```
